...
static inline __attribute__((always_inline))void event_init(void)
{
   EVENT_GPIO_REG=0;
}

static inline __attribute__((always_inline)) void event_trigger( uint8_t m )
{
   EVENT_GPIO_REG |= m;
}

static inline __attribute__((always_inline)) uint8_t event_chk( uint8_t m )
{
   return EVENT_GPIO_REG & m;
}

static inline __attribute__((always_inline)) void event_clr( uint8_t m )
{
   EVENT_GPIO_REG &= ~m;
}
#endif
